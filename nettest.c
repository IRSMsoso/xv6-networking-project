...
// Receives 1000 packets and calculates throughput
//
int
throughput_test()
{
  ...
  for(int i = 0; i < 1000; i++){
    int cc = recv(3000, &src, &sport, buf, sizeof(buf));
    if(cc < 0){
      fprintf(2, "throughput_test: recv() failed\n");
      return 0;
    }
  }

  int elapsed = uptime() - start;
  printf("Throughput: %d packets in %d ticks\n", 1000, elapsed);
...
//
// Latency test - measures round trip time
// Sends packet with timestamp, measures time until echo returns
// Calculates min/avg/max/p95/p99 latencies
//
int
latency_test()
{
  ...
  int latencies[LATENCY_SAMPLES];

  for(int i = 0; i < LATENCY_SAMPLES; i++){
    int send_time = uptime();

    // Send packet with timestamp
    char obuf[32];
    memcpy(obuf, "latency", 7);
    obuf[7] = (char)((send_time >> 24) & 0xFF);
    obuf[8] = (char)((send_time >> 16) & 0xFF);
    obuf[9] = (char)((send_time >> 8) & 0xFF);
    obuf[10] = (char)(send_time & 0xFF);

    if(send(3001, dst, dport, obuf, 11) < 0){
      fprintf(2, "latency_test: send() failed\n");
      return 0;
    }
    ...
    uint32 src;
    uint16 sport;
    int cc = recv(3001, &src, &sport, ibuf, sizeof(ibuf));
    if(cc < 0){
      fprintf(2, "latency_test: recv() failed\n");
      return 0;
    }

    int recv_time = uptime();
    latencies[i] = recv_time - send_time;

    pause(1); // Small delay between samples
  }
  ...
  int p95 = latencies[(LATENCY_SAMPLES * 95) / 100];
  int p99 = latencies[(LATENCY_SAMPLES * 99) / 100];
